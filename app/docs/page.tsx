...
function Code({ children, label }: { children: string; label: string }) {
  return (
    <figure className="mt-3">
      ...
      <pre className="mt-1.5 overflow-x-auto rounded-md bg-term p-4 font-mono text-[13px] leading-relaxed text-term-text">
        <code>{children}</code>
      </pre>
    </figure>
  );
...
export default function DocsPage() {
  return (
    <div className="mx-auto grid max-w-6xl gap-10 px-5 py-10 sm:px-8 sm:py-14 lg:grid-cols-12 [&>*]:min-w-0">
      <nav aria-label="On this page" className="hidden lg:col-span-3 lg:block">
        <ul className="sticky top-8 space-y-2 text-sm">
          {TOC.map(([id, label]) => (
            <li key={id}>
              <a href={`#${id}`} className="text-ink-soft hover:text-ink">
                {label}
              </a>
            </li>
          ))}
      ...
      <div className="min-w-0 max-w-3xl lg:col-span-9">
        <h1 className="font-display text-4xl font-semibold tracking-tight sm:text-5xl">Using Bev</h1>
        <p className="mt-4 text-lg leading-relaxed">
          Bev runs in your terminal and in your code. She asks Jev, gets the real answer in under a second, then takes
          her time. Requires Node 22 or later.
        </p>
        <div className="mt-6">
...
echo "Refund please" | npx bev-ai --json > out.json  # JSON on stdout`}</Code>
          <p className="mt-4 leading-relaxed">
            While Bev thinks, press{" "}
            <kbd className="rounded border border-ink/30 bg-card px-1.5 font-mono text-sm">r</kbd> to rush her. Each
            rush adds 10 seconds. Bev does not appreciate being rushed.
          </p>
...
result.actualMs;       // how long Jev took`}</Code>
          <p className="mt-4 leading-relaxed">
            ...
            <code className="font-mono">{"{ result, rush }"}</code> so you can rush Bev from your own UI. Options can be
            an array of labels or an object of <code className="font-mono">{"{ label: description }"}</code>. Pass{" "}
            <code className="font-mono">signal</code> to stop waiting. Errors are{" "}
            <code className="font-mono">BevError</code>s, in Bev&apos;s voice.
          </p>
        </section>
        ...
        <section className="mt-12" aria-labelledby="keys">
          ...
          <p className="mt-3 leading-relaxed">
            Bring your own Jev key. Set one of these; the first one found wins. With none, Bev runs in mock mode and
            says so.
          </p>
          ...
          <p className="mt-3 text-sm text-ink-soft">
            Your text only goes to the backend you configured. Bev stores nothing. Bev has a filing cabinet, but your
            ticket is not in it.
          </p>
        </section>
        ...
        <section className="mt-12" aria-labelledby="fax">
          ...
          <p className="mt-3 leading-relaxed">
            The fax endpoint has been retired. Bev is still upset about it. Existing faxes will be answered in the order
            they were received, which is to say eventually.
          </p>
          <p className="mt-5 border-l-4 border-sticky-edge bg-sticky/40 px-4 py-3 text-sm">
            The real decision comes from Jev by TypeSafe AI, which has actual docs and an actual API. If you need the
            answer quickly, use Jev directly. Bev will understand.
          </p>
        </section>
      </div>
    </div>
